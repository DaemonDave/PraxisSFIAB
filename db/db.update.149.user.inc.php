...
function db149_user_valid_type($type)
{
	global $user_types;
	if(is_array($type)) {
		foreach($type as $t) {
...
function db149_user_generate_password($pwlen=8)
{
 ...
 * calls if required */
function db149_user_load_fair(&$u)
{
...
function db149_user_load_committee(&$u)
{
...
function db149_user_load_volunteer(&$u)
{
...
function db149_user_load_sponsor(&$u)
{
...
function db149_user_load_principal(&$u)
{
	return true;
}

function db149_user_load_teacher(&$u)
{
	return true;
}

function db149_user_load_mentor(&$u)
{
	return true;
}
function db149_user_load_parent(&$u)
{
	return true;
}

function db149_user_load_alumni(&$u)
{
	return true;
}

function db149_user_load($user, $uid = false)
{
...
function db149_user_load_by_uid($uid)
{
	return db149_user_load(0, $uid);
}
...
function db149_user_set_password($id, $password = NULL)
{
...
function db149_user_save_type_list($u, $db, $fields)
{
...
function db149_user_save_volunteer($u)
{
	$fields = array('volunteer_active','volunteer_complete');
	db149_user_save_type_list($u, 'users_volunteer', $fields);
}

function db149_user_save_committee($u)
{
	$fields = array('committee_active','emailprivate','ord','displayemail','access_admin',
			'access_config','access_super');
	db149_user_save_type_list($u, 'users_committee', $fields);
}

function db149_user_save_judge($u)
{
	...
	db149_user_save_type_list($u, 'users_judge', $fields);
}

function db149_user_save_student($u)
{
//	$fields = array('student_active','student_complete');
//	user_save_type_list($u, 'users_student', $fields);
}

function db149_user_save_fair($u)
{
	$fields = array('fair_active','fairs_id');
	db149_user_save_type_list($u, 'users_fair', $fields);
}

function db149_user_save_sponsor($u)
{
	$fields = array('sponsors_id','sponsor_active','sponsor_complete','primary','position','notes');
	db149_user_save_type_list($u, 'users_sponsor', $fields);
}

function db149_user_save_teacher($u)
{
}

function db149_user_save_principal($u)
{
}

function db149_user_save_mentor($u)
{
}

function db149_user_save_alumni($u)
{
}

function db149_user_save_parent($u)
{
}

function db149_user_save(&$u)
{
	/* Add any new types */
	$added = array_diff($u['types'], $u['orig']['types']);
	foreach($added as $t) {
		if(!db149_user_add_role_allowed($t, $u)) {
...
//		echo "query=[$query]";
	...
	if($u['password'] != $u['orig']['password']) 
		db149_user_set_password($u['id'], $u['password']);

	/* Save types */
	foreach($u['types'] as $t) {
		call_user_func("db149_user_save_$t", $u);
	}
...
function db149_user_delete_judge($u)
{
	global $config;
	$id = $u['id'];
...
function db149_user_delete_student($u)
{
}

function db149_user_delete_sponsor($u)
{
}

function db149_user_delete_principal($u)
{
}

function db149_user_delete_teacher($u)
{
}

function db149_user_delete_parent($u)
{
}

function db149_user_delete_mentor($u)
{
}

function db149_user_delete_alumni($u)
{
}


function db149_user_delete($u, $type=false)
{
	$finish_delete = false;
	...
	if($type != false) {
		...
		if(is_array($u['types'])) {
			foreach($u['types'] as $t) call_user_func("db149_user_delete_$t", $u);
		}
		$finish_delete = true;
	}
...
function db149_user_purge($u, $type=false)
{
	$finish_purge = false;
...
function db149_user_dupe_row($db, $key, $val, $newval)
{
	global $config;
...
function db149_user_dupe($u, $new_year)
{
	/* Dupe a user if:
	...
	foreach($u2['types'] as $t) {
		db149_user_dupe_row("users_$t", 'users_id', $u['id'], $id);
	}
...
function db149_user_add_role_allowed($type, $u)
{
...
function db149_user_create($type, $username, $u = NULL)
{
	global $config;
	if(!is_array($u)) {
		 ...
		 * just add a role */
		$uid = $u['uid'];
		if(!db149_user_add_role_allowed($type, $u)) {
...
function db149_user_valid_user($user)
{
...
function db149_user_valid_password($pass)
{
...
function db149_user_valid_email($str)
{
 ...
 */
function db149_user_auth_required($type, $access='')
{
	global $config;
...
function db149_user_volunteer_registration_status()
{
	global $config;
...
function db149_user_judge_registration_status()
{
	global $config;
	$now = date('Y-m-d H:i:s');
        if($now < $config['dates']['judgeregopen']) return "notopenyet";
...
function db149_user_personal_fields($type)
{
...
function db149_user_personal_required_fields($type)
{
	global $config, $user_personal_fields_map;
	$ret = array('firstname','lastname','email');
...
function db149_user_personal_info_status(&$u)
{
	$required = array();
	foreach($u['types'] as $t) {
		$required = array_merge($required, 
				db149_user_personal_required_fields($t));
	}
	...
	/* FIXME: somehow call the $type _status_update() function db149_to update
	 * the individual $type _complete entry? */
	return 'complete';
}

/* user_{$type}_login() is called with a full $u loaded */
function db149_user_committee_login($u)
{
...
function db149_user_fair_login($u) 
{
	/* Double check, make sure the user is of this type */
